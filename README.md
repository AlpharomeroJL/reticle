...
rather than by the model's word. An MCP server offers that surface to any model host,
a 63-task benchmark scores it, and a rate-limited demo server runs it live and streams
each step to a room you can watch.

<p align="center">
  <img src="assets/agent.gif" alt="An agent proposing layout edits, a DRC violation flagged, then corrected to clean" width="70%" />
</p>

The agent proposes edits, the design-rule checker flags a violation (red), and the
loop corrects it until the check passes. Above is the real render path replaying the
transcript, the same one the in-app replay theater plays.
...
> Status: v5.0.0 adds the agent layer on top of the v4.0.0 engine: a serializable
> command API, a propose-verify-correct harness (real model or deterministic mock),
> an MCP server, a 63-task graded benchmark, live collaboration for the agent, and a
> rate-limited demo server with a Playwright end-to-end suite. The v4.0.0 engine
> brought the interactive GPU canvas on screen (10,000,000 leaf shapes at about 113
> fps, up from 6.1), a GPU-driven draw list, a minimap, split viewports, and a 3D
> layer-stack view. The workspace is green under the local gate (`just ci`, plus
> `just e2e`). For a skeptical, audited account of exactly what is fully implemented,
> what is partial, and how to verify each claim yourself, see
...
- Agent and automation: a serializable command API over the engine
  (`reticle-agent-api`, 25 commands with stable element ids and replayable
  transcripts), a propose-verify-correct harness that grades a model against the
  SKY130 DRC subset and connectivity intent (`reticle-agent`), an MCP server exposing
  every command plus three context tools (`reticle-mcp`), a 63-task graded benchmark
  (`reticle-bench`), and a rate-limited demo server that runs the loop live and streams
  each step to a watchable room (`reticle-demo-server`). See the
  [agent](docs/src/agent.md) and [benchmark](docs/src/benchmarks.md) chapters.
...
# Rate-limited demo server: the propose-verify-correct loop behind submit/status/
# cancel, streaming each step to a watchable room. Offline scripted agent with no
# key; set ANTHROPIC_API_KEY for the live model.
just demo-up

# Score the agent across the 63-task graded benchmark (deterministic mock by default).
just bench-agent

# End-to-end browser tests: a WebGL2 gate plus a WebGPU-flagged run.
just e2e
```
...
- Agent and verification: a serializable command API exposes every edit, and a
  ...
  correctness is graded rather than asserted. Every run writes a replayable transcript
  with a document hash, so a result is deterministic and auditable; the agent's edits
