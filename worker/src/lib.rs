...
impl DurableObject for ReticleRoom {
    ...
    async fn fetch(&self, req: Request) -> Result<Response> {
        ...
        // Replay the full room log to this joiner, in order, before live traffic.
    ...
    async fn websocket_message(
        ...
        if bytes.first() == Some(&PRESENCE_TAG) {
            // Coalesce: keep only the newest presence per sender; the alarm
            // flushes it within the window.
            set_pending(&storage, conn.id, &bytes).await?;
            ...
            // Updates and comments: record in the log and broadcast immediately.
    ...
    async fn alarm(&self) -> Result<Response> {
        ...
        for (sender_id, frame) in &pending {
            broadcast_except(&self.state, *sender_id, frame);
        }
        if !pending.is_empty() {
...
/// Appends one accepted frame to the room's chunked log.
...
/// Records `bytes` as the newest pending presence for `id`.
async fn set_pending(storage: &Storage, id: u64, bytes: &[u8]) -> Result<()> {
    let mut pending: Vec<(u64, Vec<u8>)> = get_or(storage, "pending").await;
    match pending.iter_mut().find(|(k, _)| *k == id) {
