...
//!
//! A few menu items are not registry commands: the dynamic **Open Recent** list,
//! the web **Convert GDS** picker (owned by lane 3B; reached here through the
//! existing web-shell call until 3B registers `file.convert_gds`), and **Take the
//! tour** (owned by lane 4C's `help.tour`; kept as a bridge so the shipped tour
//! stays relaunchable in isolation). These are injected at render time and are the
//! documented exceptions to "every menu item is a registry id"; the parity test
//! covers the registry-derived tree only.
...
pub enum MenuChoice {
    ...
    OpenRecent,
    /// Relaunch the guided tour (bridge for lane 4C's `help.tour`).
    RelaunchTour,
...
fn render_menu_content(
    ...
    match menu.label {
        ...
        "Help" => {
            ui.separator();
            if egui::Button::new("Take the tour").ui(ui).clicked() {
                *choice = Some(MenuChoice::RelaunchTour);
            }
        }
        _ => {}
    }
