...
impl eframe::App for App {
    ...
    fn ui(&mut self, ui: &mut egui::Ui, frame: &mut eframe::Frame) {
        ...
        let (canvas_screen, tour_targets) = if self.embed {
            ...
            self.main_panels(ui, gpu_format)
        };
        ...
        if !minimal_chrome {
            ...
            // The floating 3D-stack and cross-section windows are editor panels (2C);
            // the clean viewer chrome omits them, and a demo capture shows 3D only when
            // a `view3d on` step asked for it so non-3D tours stay clean.
            #[cfg(not(target_arch = "wasm32"))]
            let show_view3d = !self.is_viewer() && (!self.in_demo_capture() || self.demo_show_3d);
            #[cfg(target_arch = "wasm32")]
            let show_view3d = !self.is_viewer();
            if show_view3d {
                self.view3d.show(
                    &ctx,
                    frame,
            ...
            if !self.is_viewer() && !self.in_demo_capture() {
        ...
        // everything else. Suppressed in minimal-chrome modes and during a demo capture
        // so onboarding chrome does not cover the feature under test.
