...
impl App {
    ...
    fn handle_shortcuts(&mut self, ctx: &egui::Context) {
        ...
        if ctx.input(|i| i.key_pressed(egui::Key::Escape)) {
            // Esc cancels an in-progress draw (item 52), then closes the palette and the
            // transform popover. (The full Esc cascade contract is lane 3C, catalog 84;
            // this handles the canvas-draw arm 3A owns.)
    ...
    fn palette_window(&mut self, ctx: &egui::Context, screen: Option<ScreenRect>) {
        ...
        let layer_names: Vec<String> = self
            ...
            .map(|r| r.name.clone())
            .collect();
        let mut entries = command::catalog(&layer_names);
        // Dynamic recall entries for the saved view bookmarks (item 34): one per slot.
        for i in 0..self.bookmarks.len() {
            entries.push(command::CommandEntry::new(
                format!("View: Recall bookmark {}", i + 1),
                Command::RecallBookmark(i),
            ));
        }
    ...
    fn handle_select_input(
                ...
                (Self::drag_origin(response), response.hover_pos())
        {
            ...
            if band.width() > 0 && band.height() > 0 {
                ...
                // Same-layer filter: the layer of the shape under the drag origin, if any.
...
impl eframe::App for App {
    fn ui(&mut self, ui: &mut egui::Ui, frame: &mut eframe::Frame) {
        ...
        // The numeric transform popover (item 51), over the canvas.
