...
impl TouchMode {
    ...
    /// Resolves the tri-state to the concrete on/off that lane 4B's touch layer
    /// applies, given whether the platform currently reports a coarse pointer.
    ///
    /// `On`/`Off` override the signal; `Auto` follows it. This is the bridge between
    /// this lane's persisted preference and lane 4B's bool-based theme apply, which
    /// raises `interact_size.y` to the touch floor when the effective value is on.
    #[must_use]
    pub fn effective(self, coarse_pointer: bool) -> bool {
        match self {
            Self::On => true,
            Self::Off => false,
            Self::Auto => coarse_pointer,
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn touch_effective_resolves_auto_from_signal() {
        // On/Off ignore the coarse-pointer signal; Auto follows it.
        assert!(TouchMode::On.effective(false));
        assert!(!TouchMode::Off.effective(true));
        assert!(TouchMode::Auto.effective(true));
        assert!(!TouchMode::Auto.effective(false));
    }
}
