...
//! `F5_INDEX_JSON`'s path to the real committed index once that lane merges.
...
/// The embedded F5 plugin index: the real committed index
/// (`library/plugins/index.json`), generated by `xtask plugin-index` from the
/// committed `plugins/` sources (ADR 0121). Embedded at build time so the
/// browser browse view lists the actual shipped plugins without a fetch.
const F5_INDEX_JSON: &str = include_str!("../../../library/plugins/index.json");
...
impl PluginPanelState {
    ...
    #[must_use]
    pub fn new() -> Self {
        let index: Index = serde_json::from_str(F5_INDEX_JSON).expect(
