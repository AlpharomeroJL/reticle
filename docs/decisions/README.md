...
| [0078](0078-layout-diff-overlay.md) | Layout diff (placeholder number): a pure `reticle-diff` crate keyed on exact geometry over the flattened top cell, an app overlay (added/removed/changed) fed by a two-snapshot flow; `changed` and a file loader deferred |
