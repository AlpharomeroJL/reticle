...
| Document switch mid-live-session | GAP (ledgered) | opening/installing a document resets all per-document editor state (archive browse, layer table, selection, DRC/diff caches, old-doc overlays and pane cameras -- lane fix-doc-switch-reset), but deliberately does NOT reset the live-share mirror (`sharer_doc`, `published_revision`). Switching documents while a live session is active therefore reconciles the new document into the old CRDT, so viewers see a merge of the two. Resetting live-collaboration state on a switch is riskier than the bug it would close and was left out of scope. | lane fix-doc-switch-reset; doc-lifecycle finding |
| Derived-view invalidation is hand-wired | GAP (next campaign) | the pick index, GPU draw scene, layer table, and archive browser are each re-derived on a hand-chosen trigger, with no single owner of their agreement. v8.2.1 Wave B fixed the three known lesions pointwise (RC1/RC2/RC3), but a newly added derived view still depends on a developer wiring its invalidation by hand. The architectural fix (a document-scoped state container or one owned revision/epoch every view gates on) is deferred to the `arch/owned-invalidation` campaign, with the Wave B tests as its regression safety net. | ADR 0124; DEFECT-MAP.md |
| Browser-level test coverage | GAP (follow-on) | until v8.2.1 the app had no browser test: the suite, oracle, and pixel tests never loaded the app in a browser and drove it, which is why the Wave B root causes shipped. e2e-revive added a headless Playwright gate (`e2e-ci` in `just ci`) with the RC1 doc-switch acceptance test, the first browser regression guard. A fuller harness that loads the deployed bundle and exercises switch/zoom/pick/verify across every start-screen example is a follow-on; the headed demo-* projects need a foreground browser (eframe pauses rAF in a backgrounded tab) and are not in the headless gate. | ADR 0124; lane e2e-revive |
| Sparse-lattice overview render | GAP / v8.2.1 backlog | a sparse regular lattice (the featured 3.01 GiB streaming die is one) renders at overview zoom as a correct-but-sparse LOD point-scatter: individual cells are sub-pixel so only their anchor points paint, and zooming in lands in the wide gaps between cells rather than on filled geometry. It is correct level-of-detail behavior, but it reads as unrendered/broken to a user, not just in the README hero GIF. Planned v8.2.1 improvement: for sparse regions, LOD-draw each cell bounding box tinted by its dominant layer so the overview reads as structured colored geometry instead of a scatter. Not built. | media polish session 2 (2026-07-12); scratch/campaign/phase5-prep/media-gate-and-starry-diagnosis.md |
...
| Uniqueness claims (narrowed + dated) | OPERATOR-OWNED / re-scoped 2026-07-12 | The two BROAD claims are NOT asserted. "No other browser-native IC layout editor" is REFUTED by Layout Studio (layoutstudio.org, a live browser GDS/OASIS editor that draws shapes and runs a basic minimum-spacing / feature-size DRC on the open file). The broad benchmark claim is CHALLENGED: physically-verified layout-agent benchmarks exist (for example PDAgent-Bench, arXiv 2606.17253) without a public leaderboard. In their place the README "How it compares" section makes TWO NARROW, DATED (2026-07-12), scoped absence-of-evidence differentiators, each phrased as such with a counterexample invite: (1) no browser tool FOUND combines all three of editing, incremental microsecond-latency DRC, and multi-GiB network streaming (GDSJam is a view-only collaborative GDS viewer; Layout Studio has editing + basic DRC but opens local files by drag-drop, with no multi-GiB HTTP-Range streaming and a run-and-show DRC that is not microsecond-incremental; both verified against their own stated capabilities on 2026-07-12); (2) no OTHER physically-verified layout-agent benchmark with a PUBLIC leaderboard found. STATUS.md, docs/src/positioning.md, and the paper make no uniqueness claim; the differentiators live only in the README and are recorded here. | reticle-claims; README.md "How it compares"; layoutstudio.org; gdsjam.com; arXiv 2606.17253; scratch/campaign/phase5-prep/claims-reverification-2026-07-12.md |
