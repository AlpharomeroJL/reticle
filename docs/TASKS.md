...
- [x] Lane 4C: honest plan + worked example. done-gate-green (plan 7692f0b orchestrator
  direct; example lane 2be8294 merged, `just ci` GREEN). The plan `docs/src/tapeout.md`
  (GDS-mode versus HDL, TTSKY26c dates, pricing, tile template constraints, the precheck
  as external oracle) plus the WORKED EXAMPLE: `examples/tapeout/tt_um_reticle_tile.gds`,
  the 4A template with a `reticle_gen` serpentine test structure placed in the interior
  (met2, off met5, clear of pins/straps), built through the frozen agent surface
  (`ImportGds` + `RunGenerator` + `TransformShapes`) so it has a committed replayable
  transcript (`tt_um_reticle_tile.transcript.jsonl`); DRC-subset-clean, asserted and
  re-verified from disk; regenerated by `xtask tapeout-example`. ADR 0055. HONEST scope:
  the tile is generator-built (NOT agent-authored, the CLI is unauthenticated here) and
  DRC-clean against the SKY130 SUBSET, NOT yet run through the real precheck, which is
  the operator's step: `just tt-precheck examples/tapeout/tt_um_reticle_tile.gds`. No
  tile in the repo is claimed to pass the precheck. A paid submission is a separate
  operator decision.

**Wave 4 complete: the tape-out oracle is real. A TinyTapeout technology+template bundle
(4A, validated against the published frame), `just tt-precheck` wrapping TinyTapeout's
own Magic+KLayout precheck via a pinned Docker image with a structured-failure parser
and agent-loop seam (4B, fixture-tested; the live multi-GB run is the operator's step),
and a committed generator-built worked tile with its transcript, DRC-subset-clean (4C).
