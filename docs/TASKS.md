...
- [x] Lane R3: canvas text labels, minimap, split viewports, rebindable keys, benches doc fix. done-gate-green @ lane/r3-ui (eb42094..f6a2152), parked to merge LAST (after R2 and R4). Delivered: egui-painter text overlay (glyphon confirmed unnecessary), minimap with click-to-recenter, multi-viewport split panes, TOML keymap + editor with conflict detection, benches doc-comment fix. 49 new tests (161/161); clippy + doc-build + wasm green. app.rs additive; rebase-overlap only at canvas() head + GPU call site.
- [x] Lane R4: `stack` tech directive, 3D layer view, cut-line cross-section. done-gate-green @ lane/r4-3d-view (a37a879, 75135e0, 11bbd8b, 5021d43), parked awaiting the R1->R2->R4 merge slot. Delivered: stack directive; pipeline3d.rs + shaders/stack3d.wgsl extrusion with orbit camera; view3d.rs egui-wgpu panel; xsection.rs two-click cut-line cross-section; Tool::CutLine. 26 new tests (incl. a GPU golden run on the RTX 4060 Ti); clippy clean, wasm green. app.rs touched additively only (canvas paint path untouched); egui-wgpu added to reticle-app Cargo.toml.
