...
NEXT = WAVE 4 (diff / comments / write-multiplayer / PWA). Orientation map (exact files/APIs) in scratch/wave4-orientation.md. Adversarial review at the Wave 4 gate (V1-fixture migration correctness, selective-undo convergence, view-mode permission). Dispatch plan: 4A(diff, reticle-app+model) + 4D(PWA, web/ only) CONCURRENT now (disjoint file sets); 4B(comments+schema V1->V2) after 4A merges (app.rs collision); 4C(write-multiplayer) after 4B. 4B NON-NEGOTIABLE: its step 1 captures+commits a V1 golden fixture with the pre-V2 build BEFORE touching proto SchemaVersion. Wave 4 lanes use placeholder ADR ids -> orchestrator numbers 0078+ at merge. app-touching gates include `just wasm-build`.
